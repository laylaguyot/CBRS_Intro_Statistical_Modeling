...
```{r}
# Conduct the test
chisq.test(table(penguins_clean$species, penguins_clean$sex), correct = FALSE)
```
...
What instead of testing for the association, we would like to predict a categorical outcome?
...
You can interpret the AUC as the fact that a randomly selected male penguin has a higher predicted probability to be male than a randomly selected female penguin. On average, about 76% of the time, male penguins will have higher probabilities of being male compared to female penguins. 
...
We can find the expression of the logistic regression model with the `glm(outcome ~ predictor, family = "binomial")` function (`glm` stands for generalized linear models):
...
$\hat{p}=\frac{e^{0.0239}e^{1.099*BMI}} {{1+e^{0.0239}e^{1.099*BMI}}}$
...
For every additional 1-kg/$m^2$ increase in BMI, the odds for having diabetes are between 1.05 to 1.16 than not, at the 95% confidence level.
